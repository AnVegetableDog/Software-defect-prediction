    ...
    for i in range(n):
        for j in range(i, n):
            if trainData_Y[i] == trainData_Y[j] and trainData_Y[i] != -1:
                S[i][j] = 10
            elif (trainData_Y[i] == -1 and trainData_Y[j] > -1) and (
                    KNN.predict(trainData_X[i:i + 1]) == trainData_Y[j]):
                S[i][j] = 1
            elif (trainData_Y[j] == -1 and trainData_Y[i] > -1) and (
                    KNN.predict(trainData_X[j:j + 1]) == trainData_Y[i]):
                S[i][j] = 1
            else:
                S[i][j] = 0
            S[j][i] = S[i][j]
    ...
    while np.linalg.norm(Omega - Omega_old, ord=np.inf) > 0.001:
        ...
        if (np.linalg.norm(g_lambda[:, 0], ord=2) / u) < 0.001:
            ...
            print('gl:', np.mean(g_lambda[:, 0]), ' gl_judge:', (np.linalg.norm(
                g_lambda[:, 0], ord=2) / u), 'l_max', np.max(Lambda_vector, axis=0), 'l_min',
